...
public abstract class MyCanvas extends JPanel implements KeyListener, MouseListener {
    ...
    // The 0th column is whether or not the key is down - is it down (true) or not (false).
    // The 1st column is whether or not the key was pressed
    // The 2nd column is whether or not the key was released
    public static boolean[][] keyboardState = new boolean[525][3];
    ...
    // This method is called when the jPanel is drawn. When it is called is decided by the java system.
    // However, a paintComponent command can be suggested through the repaint function.
    @Override
    ...
    {
        keyboardState[e.getKeyCode()][0] = true;
        for (int i = 0; i < 525; i++ ) {
            keyboardState[i][1] = false;
        }
        keyboardState[e.getKeyCode()][1] = true;
        keyboardState[e.getKeyCode()][2] = false;
    }
    
    @Override
    public void keyReleased(KeyEvent e)
    {
        keyboardState[e.getKeyCode()][0] = false;
        keyboardState[e.getKeyCode()][1] = false;
        keyboardState[e.getKeyCode()][2] = true;
